...
from modules.criterion import CrossEntropyLoss2d, MultiTaskLossWrapper
...
def get_gen(train_imgs, label_imgs, label_wt_map_imgs):
    batch_sz = train_imgs.size(0)
    imageType = train_imgs.size(1)
    labelType = label_imgs.size(1)
    labelmapType = label_wt_map_imgs.size(1)
    ...
    label_wt_map_samples = np.zeros((batch_sz, labelmapType, ins, ins))

    train_samples = train_imgs[:, :, stx:stx+ins, sty:sty+ins]
    label_samples = label_imgs[:, :, stx:stx+ins, sty:sty+ins]
    label_wt_map_samples = label_wt_map_imgs[:, :, stx:stx+ins, sty:sty+ins] 

    return train_samples, label_samples, label_wt_map_samples
...
def train(args, model):
    ...
    if args.cuda:
        criter = MultiTaskLossWrapper(2, model).cuda()
    else:
        criter = MultiTaskLossWrapper(2, model)

    optimizer = Adam(criter.parameters(),lr = 0.00005)
    ...
    for epoch in range(1, args.num_epochs+1):
        ...
        epoch_loss = []
        val_epoch_loss = []
        for step, (images, labels, labels_wt_map) in enumerate(loader):
            print(images.shape)
            print(labels.shape)
            images, labels, labels_wt_map = get_gen(images, labels, labels_wt_map)
            #print(images.shape)
            #print(labels.shape)
            if args.cuda:
                images = images.cuda()
                labels = labels.cuda()
                labels_wt_map = labels_wt_map.cuda()
            inputs = Variable(images)
            targets = Variable(labels)
            targets_map = Variable(labels_wt_map)
            look = targets[targets > 1]
            
            #outputs = model(inputs)

            loss, log_vars = criter(inputs, targets[:,0], targets_map[:,0])
            print('main here')
            print(log_vars)
            print(loss)
            print(loss.item())
            ...
            if (epoch > 1000 and epoch % args.epoch_save == 0 and step ==0) or (epoch == 20 and step == 0):
        ...
        if len(val_loader) > 0:
            for step, (images, labels, labels_wt_map) in enumerate(val_loader):
                if args.cuda:
                    images = images.cuda()
                    labels = labels.cuda()
                    labels_wt_map = labels_wt_map.cuda()
                ...
                #val_loss = criterion(outputs[0], targets[:,0])
                val_loss, log_vars = criter(inputs, targets[:,0], targets_map[:,0])
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--num-epochs', type=int, default=20001)
    ...
    parser_train.add_argument('--epoch_save', type=int, default=1000)
